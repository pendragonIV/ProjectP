    ...
    public class FoodConverterAnimation : MonoBehaviour
    {
        ...
        private void OnOutStorageResourcesChanged()
        {
            ...
            else
            {
                ...
                Currency resultCurrency = CurrencyController.GetCurrency(converter.OutStorage.Storage[0].currency);
                float verticalOffset = resultCurrency.Data.DropResPrefab.GetComponent<ResourceDropBehavior>().VerticalOffset;

                resultPreview = resultCurrency.Data.FlyingResPool.GetPooledObject().SetPosition(resultPreviewSpawnPosition.position.AddToY(verticalOffset)).SetScale(Vector3.one).SetRotation(Quaternion.Euler(resultPreviewRotation));
            }
        }
    }
}